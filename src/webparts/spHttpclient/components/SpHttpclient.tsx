...
const SpHttpclient:React.FC<ISpHttpclientProps>=(props)=>{
  ...
  const [age,setAge]=React.useState("");
   ...
   const [allItems,setAllItems]=React.useState<IEmpdata[]>([]);
...
const createItems=async():Promise<void>=>{
 ...
 try{
  ...
  const response:SPHttpClientResponse=await props.context.spHttpClient.post(`{props.siteurl}/_api/web/lists/getbytitle('${ListNames.FormikList}')/items`,
...
//updates items
 const updateITems=():void=>{
    ...
    if(idElement){
      ...
      if(id>0){
        props.context.spHttpClient.post(`${props.siteurl}/_api/web/lists/getbytitle('${ListNames.FormikList}')/items(${id})`,
        SPHttpClient.configurations.v1,
        {
          headers:{
            'Accept':'application/json;odata=nometadata',
 ...
 const deleteItems=():void=>{
  ...
  const getItemBYID=():void=>{
    ...
    if(idElement?.value){
      ...
      if(id>0){
        props.context.spHttpClient.get(`${props.siteurl}/_api/web/lists/getbytitle('${ListNames.FormikList}')/items(${id})`,
        SPHttpClient.configurations.v1,
        {
          headers:{
            'Accept':'application/json;odata=nometadata',
...
const getALLitems=():void=>{
  props.context.spHttpClient.get(`${props.siteurl}/_api/web/lists/getbytitle('${ListNames.FormikList}')/items`,
  SPHttpClient.configurations.v1,
  {
    headers:{
      'Accept':'application/json;odata=nometadata',
